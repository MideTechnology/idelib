# Helper functions for implementing Welch's method FFT-alikes (PSD/CSD and spectrogram). These functions are already present in (and directly copied from) matplotlib;
#   they have been pulled into a separate callable file to avoid auto-spawning a Figure when called. Also, several bugs affecting amplitude scaling have been fixed here, 
#   so these should be used in place of the matplotlib functions wherever a proper amplitude scaling is required.

# ALSO NOTE: I modified _spectral_helper to return amplitude spectra (sqrt(x*conj(x)) rather than power spectra (x*conj(x)).

# This file is part of Slam Stick Viewer.
...
def window_hanning(x):
    ...
    #print "In window_hanning: len x is " % len(x)
    #print "In window_hanning: x is "
    #print x
    #print np.size(x)
...
#     print "Window: "
#     print windowVals
#     print "Window sum: %f" % np.abs(windowVals).sum()
#     print "Window norm: %f" % (np.abs(windowVals).sum() / np.size(windowVals))
    # matplotlib BUG: want to scale by the average of the window coefs, not the sum!
...
#     print "Fs: %d   Scaling_factor: %f   Inv. scaling: %f\r\n" % (Fs, scaling_factor, 1./scaling_factor)
