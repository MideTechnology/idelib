...
builds = (
    r'C:\Python27\Scripts\pyinstaller.exe --noconfirm --onefile --distpath="%(dist_32)s" -i .\ssx.ico ide2csv-win-onefile.spec',
    r'c:\Python27_64\Scripts\pyinstaller --noconfirm --onefile --distpath="%(dist_64)s" --workpath=build_64 -i .\ssx.ico ide2csv-win-onefile.spec',
)
...
def writeInfo(version, debug, buildNum, buildTime, buildMachine):
    with open('build_info_ide2csv.py', 'wb') as f:
...
try:
    ...
    from build_info_ide2csv import BUILD_NUMBER, DEBUG, VERSION, BUILD_TIME, BUILD_MACHINE
...
buildArgs = {
    'dist_32': 'ide2csv v%s.%04d%s' % (versionString, thisBuildNumber, ' experimental' if thisDebug else ''),
    'dist_64': 'ide2csv v%s.%04d%s' % (versionString, thisBuildNumber, ' experimental' if thisDebug else ''),
}
