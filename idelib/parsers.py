...
# Add special-case dummy NMEA data parser. This will eventually be replaced
# with a 'real' parser.
DATA_PARSERS['NMEA'] = struct.Struct('c')
...
class ChannelParser(ElementHandler):
    ...
    def parse(self, element, **kwargs):
        ...
        if 'parser' in data:
            if data['parser'] in DATA_PARSERS:
                # A named parser; use the special-case parser function.
                data.pop('format', None)
                data['parser'] = DATA_PARSERS[data['parser']]
            else:
                warnings.warn('Unknown parser type or for channel {}: {!r}'.format(channelId, data['parser']))
                data['parser'] = struct.Struct('c')
        elif 'format' in data:
            # build struct instead.
            # future: Handle special (non-standard) format characters (TBD)
            data['parser'] = struct.Struct(data.pop('format'))
        else:
            warnings.warn('Missing parser/format definition for channel {}'.format(channelId))
            data['parser'] = struct.Struct('c')
